...
void _configureTimerFrequency(long pwm_frequency, mcpwm_dev_t* mcpwm_num,  mcpwm_unit_t mcpwm_unit, float dead_zone = NOT_SET){
  ...
  if (_isset(dead_zone)){
    ...
    // Note: This is basically unused now
    pwm_mode = MCPWM_DEADTIME_BYPASS;
...
void* _configure6PWM(long pwm_frequency, float dead_zone, const int pinA_h, const int pinA_l,  const int pinB_h, const int pinB_l, const int pinC_h, const int pinC_l){
  ...
  // mcpwm_set_signal_high(m_slot.mcpwm_unit, MCPWM_TIMER_0, MCPWM_GEN_B); 
  // mcpwm_set_signal_high(m_slot.mcpwm_unit, MCPWM_TIMER_1, MCPWM_GEN_B);
  // mcpwm_set_signal_high(m_slot.mcpwm_unit, MCPWM_TIMER_2, MCPWM_GEN_B);

  // mcpwm_set_signal_low(m_slot.mcpwm_unit, MCPWM_TIMER_0, MCPWM_GEN_A); 
  // mcpwm_set_signal_low(m_slot.mcpwm_unit, MCPWM_TIMER_1, MCPWM_GEN_A);
  // mcpwm_set_signal_low(m_slot.mcpwm_unit, MCPWM_TIMER_2, MCPWM_GEN_A);
...
void _writeDutyCycle6PWM(float dc_a,  float dc_b, float dc_c, PhaseState *phase_state, void* params){
      ...
      mcpwm_set_duty(((ESP32MCPWMDriverParams*)params)->mcpwm_unit, MCPWM_TIMER_0, MCPWM_OPR_A, (phase_state[0] == PHASE_ON || phase_state[0] == PHASE_HI) ? _constrain(dc_a-((ESP32MCPWMDriverParams*)params)->deadtime,0.0,1.0)*100.0 : 0);
      mcpwm_set_duty(((ESP32MCPWMDriverParams*)params)->mcpwm_unit, MCPWM_TIMER_0, MCPWM_OPR_B, (phase_state[0] == PHASE_ON || phase_state[0] == PHASE_LO) ? _constrain(dc_a+((ESP32MCPWMDriverParams*)params)->deadtime,0.0,1.0)*100.0 : 0);
      mcpwm_set_duty(((ESP32MCPWMDriverParams*)params)->mcpwm_unit, MCPWM_TIMER_1, MCPWM_OPR_A, (phase_state[1] == PHASE_ON || phase_state[1] == PHASE_HI) ? _constrain(dc_b-((ESP32MCPWMDriverParams*)params)->deadtime,0.0,1.0)*100.0 : 0);
      mcpwm_set_duty(((ESP32MCPWMDriverParams*)params)->mcpwm_unit, MCPWM_TIMER_1, MCPWM_OPR_B, (phase_state[1] == PHASE_ON || phase_state[1] == PHASE_LO) ? _constrain(dc_b+((ESP32MCPWMDriverParams*)params)->deadtime,0.0,1.0)*100.0 : 0);
      mcpwm_set_duty(((ESP32MCPWMDriverParams*)params)->mcpwm_unit, MCPWM_TIMER_2, MCPWM_OPR_A, (phase_state[2] == PHASE_ON || phase_state[2] == PHASE_HI) ? _constrain(dc_c-((ESP32MCPWMDriverParams*)params)->deadtime,0.0,1.0)*100.0 : 0);
      mcpwm_set_duty(((ESP32MCPWMDriverParams*)params)->mcpwm_unit, MCPWM_TIMER_2, MCPWM_OPR_B, (phase_state[2] == PHASE_ON || phase_state[2] == PHASE_LO) ? _constrain(dc_c+((ESP32MCPWMDriverParams*)params)->deadtime,0.0,1.0)*100.0 : 0);
      // for (size_t i = 0; i < 3; i++)
      // {
      //   if (phase_state[i] == PHASE_OFF)
      //   {
      //       mcpwm_set_signal_low(((ESP32MCPWMDriverParams*)params)->mcpwm_unit, (mcpwm_timer_t) i, MCPWM_GEN_A);
      //       mcpwm_set_signal_high(((ESP32MCPWMDriverParams*)params)->mcpwm_unit, (mcpwm_timer_t) i, MCPWM_GEN_B); 
      //   } else if (phase_state[i] == PHASE_ON)
      //   { 
      //       mcpwm_set_duty_type(((ESP32MCPWMDriverParams*)params)->mcpwm_unit, (mcpwm_timer_t) i, MCPWM_GEN_B, MCPWM_DUTY_MODE_0);
      //       mcpwm_set_duty_type(((ESP32MCPWMDriverParams*)params)->mcpwm_unit, (mcpwm_timer_t) i, MCPWM_GEN_A, MCPWM_DUTY_MODE_0);
      //   } else if (phase_state[i] == PHASE_HI)
      //   { 
      //       mcpwm_set_signal_high(((ESP32MCPWMDriverParams*)params)->mcpwm_unit, (mcpwm_timer_t) i, MCPWM_GEN_B); 
      //       mcpwm_set_duty_type(((ESP32MCPWMDriverParams*)params)->mcpwm_unit, (mcpwm_timer_t) i, MCPWM_GEN_A, MCPWM_DUTY_MODE_0);
      //   }
